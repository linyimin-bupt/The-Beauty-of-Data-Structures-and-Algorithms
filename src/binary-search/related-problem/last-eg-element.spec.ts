...
import { BSGetLastLeElement   } from './last-eg-element'

test ('binary search get last small or equal element', async t => {
  const arr = generateArray (100000, 0)
  mergeSort (arr)
  if (! isSorted (arr)) {
    throw new Error ('binary search only be used in an order array')
  }
  const result = BSGetLastLeElement (arr, 100)
  const EXPECTED = 100
  
  t.equal (result, EXPECTED, 'binary search get last small or equal element')
})
